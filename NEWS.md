...
- We now support a choice between fast, exact, and adaptive predicates via `Fast()`, `Exact()`, and `def_alg222()`, respectively. The default is now `def_alg222()`. Moreover, triangle areas are now computed using the adaptive `orient` predicate to be more robust. See [#165](https://github.com/JuliaGeometry/DelaunayTriangulation.jl/pull/165).
...
`DelaunayTriangulation.Fast()`, `DelaunayTriangulation.Exact()`, or `DelaunayTriangulation.def_alg222()`. If you are computing a predicate manually, then the predicate is instead passed as the first argument.
