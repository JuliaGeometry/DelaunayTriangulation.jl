@doc """
    triangle_orientation([method::AbstractPredicateType=def_alg222(),] tri::Triangulation, i, j, k) -> Certificate 
    triangle_orientation([method::AbstractPredicateType=def_alg222(),] tri::Triangulation, T) -> Certificate
    triangle_orientation([method::AbstractPredicateType=def_alg222(),] p, q, r) -> Certificate
...
function triangle_orientation(method::AbstractPredicateType, p, q, r)
    cert = orient_predicate(method, p, q, r)
...
function triangle_orientation(method::AbstractPredicateType, tri::Triangulation, i, j, k)
    if is_exterior_ghost_triangle(tri, i, j, k)
        i, j, k = k, j, i
    end
    p, q, r = get_point(tri, i, j, k)
    return triangle_orientation(method, p, q, r)
end
triangle_orientation(tri::Triangulation, i, j, k) = triangle_orientation(def_alg222(), tri, i, j, k)
triangle_orientation(method::AbstractPredicateType, tri::Triangulation, T) = triangle_orientation(method, tri, geti(T), getj(T), getk(T))
triangle_orientation(tri::Triangulation, T) = triangle_orientation(def_alg222(), tri, T)

"""
    point_position_relative_to_circle([method::AbstractPredicateType=def_alg222(),] a, b, c, p) -> Certificate
...
See the documentation for more information about these choices.
"""
function point_position_relative_to_circle(method::AbstractPredicateType, a, b, c, p)
    cert = incircle_predicate(method, a, b, c, p)
...
@doc """
    point_position_relative_to_line([method::AbstractPredicateType=def_alg222(),] a, b, p) -> Certificate
    point_position_relative_to_line([method::AbstractPredicateType=def_alg222(),] tri::Triangulation, i, j, u) -> Certificate
...
function point_position_relative_to_line(method::AbstractPredicateType, a, b, p)
    cert = orient_predicate(method, a, b, p)
...
function point_position_relative_to_line(method::AbstractPredicateType, tri::Triangulation, i, j, u)
    a, b, p = get_point(tri, i, j, u)
    if is_exterior_ghost_edge(tri, i, j)
        return point_position_relative_to_line(method, b, a, p)
    else
        return point_position_relative_to_line(method, a, b, p)
    end
end
point_position_relative_to_line(tri::Triangulation, i, j, u) = point_position_relative_to_line(def_alg222(), tri, i, j, u)

@doc """
    point_closest_to_line([method::AbstractPredicateType=def_alg222(),] a, b, p, q) -> Certificate
    point_closest_to_line([method::AbstractPredicateType=def_alg222(),] tri::Triangulation, i, j, u, v) -> Certificate
...
function point_closest_to_line(method::AbstractPredicateType, a, b, p, q)
    cert = parallelorder_predicate(method, a, b, q, p)
...
function point_closest_to_line(method::AbstractPredicateType, tri::Triangulation, i, j, u, v)
    a, b, p, q = get_point(tri, i, j, u, v)
    return point_closest_to_line(method, a, b, p, q)
end
point_closest_to_line(tri::Triangulation, i, j, u, v) = point_closest_to_line(def_alg222(), i, j, u, v)
...
@doc """
    line_segment_intersection_type([method::AbstractPredicateType=def_alg222(),] p, q, a, b) -> Certificate 
    line_segment_intersection_type([method::AbstractPredicateType=def_alg222(),] tri::Triangulation, u, v, i, j) -> Certificate
...
function line_segment_intersection_type(method::AbstractPredicateType, p, q, a, b)
    cert = meet_predicate(method, p, q, a, b)
    ...
    if is_multiple(converted_cert)
        ...
        =#
        collinear_cert_1 = point_position_relative_to_line(method, a, b, p)
        collinear_cert_2 = point_position_relative_to_line(method, a, b, q)
        ...
        collinear_cert_3 = point_position_relative_to_line(method, p, q, a)
        collinear_cert_4 = point_position_relative_to_line(method, p, q, b)
...
function line_segment_intersection_type(method::AbstractPredicateType, tri::Triangulation, u, v, i, j)
    p, q, a, b = get_point(tri, u, v, i, j)
    return line_segment_intersection_type(method, p, q, a, b)
end
line_segment_intersection_type(tri::Triangulation, u, v, i, j) = line_segment_intersection_type(def_alg222(), tri, u, v, i, j)

@doc """
    point_position_relative_to_triangle([method::AbstractPredicateType=def_alg222(),] a, b, c, p) -> Certificate
    point_position_relative_to_triangle([method::AbstractPredicateType=def_alg222(),] tri::Triangulation, i, j, k, u) -> Certificate
    point_position_relative_to_triangle([method::AbstractPredicateType=def_alg222(),] tri::Triangulation, T, u) -> Certificate
...
function point_position_relative_to_triangle(method::AbstractPredicateType, a, b, c, p)
    edge_ab = point_position_relative_to_line(method, a, b, p)
    edge_bc = point_position_relative_to_line(method, b, c, p)
    edge_ca = point_position_relative_to_line(method, c, a, p)
...
function point_position_relative_to_triangle(method::AbstractPredicateType, tri::Triangulation, i, j, k, u)
    if !is_exterior_ghost_triangle(tri, i, j, k)
        a, b, c, p = get_point(tri, i, j, k, u)
        return point_position_relative_to_triangle(method, a, b, c, p)
    else
        ...
        edge_bc = point_position_relative_to_line(method, c, b, p) # Flipped to match centroid location
...
point_position_relative_to_triangle(method::AbstractPredicateType, tri::Triangulation, T, u) = point_position_relative_to_triangle(method, tri, geti(T), getj(T), getk(T), u)
point_position_relative_to_triangle(tri::Triangulation, T, u) = point_position_relative_to_triangle(def_alg222(), tri, T, u)

"""
    point_position_relative_to_oriented_outer_halfplane([method::AbstractPredicateType=def_alg222(),] a, b, p) -> Certificate 
...
"""
function point_position_relative_to_oriented_outer_halfplane(method::AbstractPredicateType, a, b, p)
    in_open_halfplane = point_position_relative_to_line(method, a, b, p)
...
@doc """
    is_legal([method::AbstractPredicateType=def_alg222(),] tri::Triangulation, i, j) -> Certificate 
    is_legal([method::AbstractPredicateType=def_alg222(),] p, q, r, s) -> Certificate
...
"""
is_legal
function is_legal(method::AbstractPredicateType, p, q, r, s)
    incirc = point_position_relative_to_circle(method, p, q, r, s)
    if is_inside(incirc)
        return Cert.Illegal
    else
        return Cert.Legal
...
function is_legal(method::AbstractPredicateType, tri::Triangulation, i, j)
...
@doc """
    triangle_line_segment_intersection([method::AbstractPredicateType=def_alg222(),] p, q, r, a, b) -> Certificate 
    triangle_line_segment_intersection([method::AbstractPredicateType=def_alg222(),] tri::Triangulation, i, j, k, u, v) -> Certificate
...
function triangle_line_segment_intersection(method::AbstractPredicateType, p, q, r, a, b)
    ## Break into cases:
    ...
    if a_eq || b_eq
        ...
        pqb = point_position_relative_to_line(method, p, q, b)
        prb = point_position_relative_to_line(method, p, r, b)
        ...
        elseif is_collinear(pqb) || is_collinear(prb)
            ...
            qrb = point_position_relative_to_line(method, q, r, b)
            if is_right(qrb)
    ...
    a_tri = point_position_relative_to_triangle(method, p, q, r, a)
    b_tri = point_position_relative_to_triangle(method, p, q, r, b)
    ...
    if is_on(a_tri) && is_on(b_tri)
        ...
        pqa = is_collinear(point_position_relative_to_line(method, p, q, a))
        qra = pqa ? false : is_collinear(point_position_relative_to_line(method, q, r, a)) # skip computation if we already know a result
        #rpa = (pqa || qra) ? false : is_collinear(point_position_relative_to_line(r, p, a))
        pqb = is_collinear(point_position_relative_to_line(method, p, q, b))
        qrb = pqb ? false : is_collinear(point_position_relative_to_line(method, q, r, b))
    ...
    if is_on(a_tri) || is_on(b_tri)
        ...
        pqa = is_collinear(point_position_relative_to_line(method, p, q, a))
        qra = pqa ? false : is_collinear(point_position_relative_to_line(method, q, r, a))
        rpa = !(pqa || qra)
    ...
    a_pos = point_position_relative_to_triangle(method, p, q, r, a)
    b_pos = point_position_relative_to_triangle(method, p, q, r, b)
    ...
    elseif is_inside(a_pos) || is_inside(b_pos)
        ...
        pq_ab = line_segment_intersection_type(method, p, q, a, b)
        qr_ab = line_segment_intersection_type(method, q, r, a, b)
        rp_ab = line_segment_intersection_type(method, r, p, a, b)
        ...
        elseif num_multiple == 1 || num_touching == 2
            ...
            abp = point_position_relative_to_line(method, a, b, p)
            abq = point_position_relative_to_line(method, a, b, q)
            abr = point_position_relative_to_line(method, a, b, r)
...
function triangle_line_segment_intersection(method::AbstractPredicateType, tri::Triangulation, i, j, k, u, v)
    p, q, r, a, b = get_point(tri, i, j, k, u, v)
    return triangle_line_segment_intersection(method, p, q, r, a, b)
end
...
"""
    find_edge([predicates::AbstractPredicateType=def_alg222(),] tri::Triangulation, T, ℓ) -> Edge 
...
using the `predicates` argument.
"""
function find_edge(predicates::AbstractPredicateType, tri::Triangulation, T, ℓ)
    ...
    if is_ghost_vertex(k) # must be on the solid edge
        ...
        is_collinear(point_position_relative_to_line(predicates, tri, a, b, ℓ)) && return construct_edge(E, i, j)
        is_collinear(point_position_relative_to_line(predicates, tri, b, c, ℓ)) && return construct_edge(E, j, k)
...
@doc """
    point_position_relative_to_circumcircle([method::AbstractPredicateType=def_alg222(),] tri::Triangulation, i, j, k, ℓ) -> Certificate
    point_position_relative_to_circumcircle([method::AbstractPredicateType=def_alg222(),] tri::Triangulation, T, ℓ) -> Certificate
...
function point_position_relative_to_circumcircle(method::AbstractPredicateType, tri::Triangulation, i, j, k, ℓ)
    u, v, w = sort_triangle(i, j, k)
    a, b, c, p = get_point(tri, u, v, w, ℓ)
    if is_ghost_vertex(w)
        cert = point_position_relative_to_oriented_outer_halfplane(method, a, b, p)
        if is_on(cert) && is_weighted(tri)
            u′, v′, w′ = replace_ghost_triangle_with_boundary_triangle(tri, (u, v, w))
            sub_cert = point_position_relative_to_witness_plane(method, tri, u′, v′, w′, ℓ)
    ...
    elseif !is_weighted(tri)
        return point_position_relative_to_circle(method, a, b, c, p)
    else
        cert = point_position_relative_to_witness_plane(method, tri, i, j, k, ℓ)
...
point_position_relative_to_circumcircle(method::AbstractPredicateType, tri::Triangulation, T, ℓ) = point_position_relative_to_circumcircle(method, tri, geti(T), getj(T), getk(T), ℓ)
...
"""
    point_position_relative_to_witness_plane([method::AbstractPredicateType=def_alg222(),] tri::Triangulation, i, j, k, ℓ) -> Certificate
...
"""
function point_position_relative_to_witness_plane(method::AbstractPredicateType, tri::Triangulation, i, j, k, ℓ)
    p⁺ = get_lifted_point(tri, i)
    q⁺ = get_lifted_point(tri, j)
    r⁺ = get_lifted_point(tri, k)
    a⁺ = get_lifted_point(tri, ℓ)
    cert = orient_predicate(method, p⁺, q⁺, r⁺, a⁺)
...
"""
    opposite_angle([method::AbstractPredicateType=def_alg222(),] p, q, r) -> Certificate
...
"""
function opposite_angle(method::AbstractPredicateType, p, q, r) # https://math.stackexchange.com/a/701682/861404 
    cert = angle_is_acute(method, p, q, r)
...
"""
    point_position_relative_to_diametral_circle([method::AbstractPredicateType=def_alg222(),] p, q, r) -> Certificate
...
"""
function point_position_relative_to_diametral_circle(method::AbstractPredicateType, p, q, r)
    d = opposite_angle(method, p, q, r)
    if is_obtuse(d)
...
"""
    test_visibility([method::AbstractPredicateType = def_alg222(),] tri::Triangulation, q, i) -> Certificate 
...
# Arguments 
- `method::AbstractPredicateType = def_alg222()`: Method to use for computing predicates. Can be one of [`Fast`](@ref), [`Exact`](@ref) (the default), and [`Adaptive`](@ref). See the documentation for a further discussion of these methods.
...
"""
function test_visibility(method::AbstractPredicateType, tri::Triangulation, q, i)
    V, invisible_flag = find_triangle(tri, q; use_barriers=Val(true), k=i, concavity_protection=true, predicates=method)
    if invisible_flag
...
"""
    test_visibility([method::AbstractPredicateType=def_alg222(),] tri::Triangulation, u, v, i; shift=0.0, attractor=get_point(tri,i)) -> Certificate 
...
# Arguments 
- `method::AbstractPredicateType=def_alg222()`: How predicates are computed. See the documentation for information on the choices between [`Fast`](@ref), [`Exact`](@ref), and [`Adaptive`](@ref).
- `tri`: The [`Triangulation`](@ref).
...
"""
function test_visibility(method::AbstractPredicateType, tri::Triangulation, u, v, i; shift=0.0, attractor=get_point(tri, i))
    ...
    for t in ts
        ...
        cert = test_visibility(method, tri, m̃, i)
