...
"""
    _centroidal_smooth_itr(vorn::VoronoiTessellation, set_of_boundary_nodes, points, rng, predicates::AbstractPredicateType=Adaptive(); kwargs...) -> (VoronoiTessellation, Number)
...
"""
function _centroidal_smooth_itr(vorn::VoronoiTessellation, set_of_boundary_nodes, points, rng, predicates::AbstractPredicateType=Adaptive(); kwargs...)
    F = number_type(vorn)
    max_dist = zero(F)
...
"""
    centroidal_smooth(vorn::VoronoiTessellation; maxiters=1000, tol=default_displacement_tolerance(vorn), rng=Random.default_rng(), predicates::AbstractPredicateType=Adaptive(), kwargs...) -> VoronoiTessellation
...
"""
function centroidal_smooth(vorn::VoronoiTessellation; maxiters=1000, tol=default_displacement_tolerance(vorn), rng=Random.default_rng(), predicates::AbstractPredicateType=Adaptive(), kwargs...) 
    iter = 0
    F = number_type(vorn)
    max_dist = typemax(F)
