...
"""
    process_ray_intersection!(
        ...
        predicates::AbstractPredicateKernel=def_alg222()) -> Point
...
- `predicates::AbstractPredicateKernel=def_alg222()`: Method to use for computing predicates. Can be one of [`Fast`](@ref), [`Exact`](@ref), and [`Adaptive`](@ref). See the documentation for a further discussion of these methods. Needed for [`get_nearest_neighbour`](@ref).

# Outputs
There are no outputs, as `polygon_edge_queue` is modified in-place.
"""
function enqueue_new_edge!(polygon_edge_queue, vorn::VoronoiTessellation, e, rng::Random.AbstractRNG=Random.default_rng(), predicates::AbstractPredicateKernel=def_alg222())
    u, v = edge_vertices(e)
...
"""
    process_polygon!(vorn::VoronoiTessellation, e, incident_polygon, boundary_sites, segment_intersections, intersected_edge_cache, exterior_circumcenters, equal_circumcenter_mapping, predicates::AbstractPredicateKernel=def_alg222()) -> (Edge, Edge, Edge)
...
"""
function process_polygon!(vorn, e, incident_polygon, boundary_sites, segment_intersections, intersected_edge_cache, exterior_circumcenters, equal_circumcenter_mapping, predicates::AbstractPredicateKernel=def_alg222())
...
"""
    dequeue_and_process!(vorn, polygon_edge_queue, edges_to_process,
        ...
        rng::Random.AbstractRNG=Random.default_rng(), predicates::AbstractPredicateKernel=def_alg222())
...
"""
    find_all_intersections(vorn::VoronoiTessellation; rng=Random.default_rng(), predicates::AbstractPredicateKernel=def_alg222()) -> (Dict, Vector, Set, Dict)
...
5. We then return.
"""
function find_all_intersections(vorn::VoronoiTessellation; rng::Random.AbstractRNG=Random.default_rng(), predicates::AbstractPredicateKernel=def_alg222())
    edges_to_process,
    polygon_edge_queue,
    boundary_sites,
    segment_intersections,
...
"""
    clip_voronoi_tessellation!(vorn::VoronoiTessellation, is_convex=true; rng=Random.default_rng(), predicates::AbstractPredicateKernel=def_alg222())
...
"""
function clip_voronoi_tessellation!(vorn::VoronoiTessellation, is_convex=true; rng::Random.AbstractRNG=Random.default_rng(), predicates::AbstractPredicateKernel=def_alg222())
