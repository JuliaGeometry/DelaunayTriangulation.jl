...
"""
    prepare_initial_edge(tri::Triangulation, edges, p, q, rng::Random.AbstractRNG=Random.default_rng(), predicates::AbstractPredicateType=def_alg222()) -> (Vertex, Vertex, Point, Point, Certificate, Certificate)
...
"""
function prepare_initial_edge(tri::Triangulation, edges, p, q, rng::Random.AbstractRNG=Random.default_rng(), predicates::AbstractPredicateType=def_alg222())
...
"""
function select_initial_triangle_interior_vertex(tri::Triangulation, k, q, predicates::AbstractPredicateType=def_alg222(), store_history::F=Val(false), history=nothing, rng::Random.AbstractRNG=Random.default_rng()) where {F}
    p = get_point(tri, k)
    ...
    return_flag, p, i, j, pᵢ, pⱼ, line_cert_i, line_cert_j = fix_initial_collinear_edge_for_interior_vertex(tri, k, q, predicates, store_history, history, rng, p, neighbouring_edges, i, j, pᵢ, pⱼ, line_cert_i, line_cert_j)
    ...
    if is_left(line_cert_j)
        i, j, pᵢ, pⱼ = select_initial_triangle_clockwise(tri, p, q, pᵢ, pⱼ, i, j, k, predicates, store_history, history)
    else
        i, j, pᵢ, pⱼ = select_initial_triangle_counterclockwise(tri, line_cert_j, p, q, pᵢ, pⱼ, i, j, k, predicates, store_history, history)
    end
...
function select_initial_triangle_clockwise(tri::Triangulation, p, q, pᵢ, pⱼ, i, j, k, predicates::AbstractPredicateType=def_alg222(), store_history::F=Val(false), history=nothing) where {F}
    line_cert_i = point_position_relative_to_line(predicates, p, q, pᵢ)
...
function select_initial_triangle_counterclockwise(tri::Triangulation, line_cert_j, p, q, pᵢ, pⱼ, i, j, k, predicates::AbstractPredicateType=def_alg222(), store_history::F=Val(false), history=nothing) where {F}
...
function fix_initial_collinear_edge_for_interior_vertex(tri::Triangulation, k, q, predicates::AbstractPredicateType, store_history, history, rng, p, neighbouring_edges, i, j, pᵢ, pⱼ, line_cert_i, line_cert_j)
    while is_collinear(line_cert_j) || is_collinear(line_cert_i)
        ...
        if !is_left(on_edge_cert)
            if is_on(on_edge_cert) || is_degenerate(on_edge_cert)
                ...
                pos_cert = point_position_relative_to_line(predicates, p, q, pᵢ)
                if is_left(pos_cert)
                    is_true(store_history) && add_edge!(history, k, opposing_idx)
...
"""
    check_for_intersections_with_adjacent_boundary_edges(tri::Triangulation, k, q, ghost_vertex=𝒢, predicates::AbstractPredicateType=def_alg222()) -> (Certificate, Certificate, Vertex, Certificate, Certificate)
...
"""
function check_for_intersections_with_adjacent_boundary_edges(tri::Triangulation{P,T,BN,W,I}, k, q, ghost_vertex=I(𝒢), predicates::AbstractPredicateType=def_alg222()) where {P,T,BN,W,I}
    p = get_point(tri, k)
    ...
    right_cert = point_position_relative_to_line(predicates, p, pright, q)
    left_cert = point_position_relative_to_line(predicates, pleft, p, q)
...
"""
    search_down_adjacent_boundary_edges(tri::Triangulation, k, q, direction_cert, q_pos_cert, next_vertex, predicates::AbstractPredicateType=def_alg222(), store_history=Val(false), history=nothing, ghost_vertex=𝒢) -> (Bool, Certificate, Vertex, Vertex, Vertex)
...
"""
function search_down_adjacent_boundary_edges(tri::Triangulation, k, q, direction, q_pos, next_vertex, predicates::AbstractPredicateType=def_alg222(), store_history::F=Val(false), history=nothing, ghost_vertex=integer_type(tri)(𝒢)) where {F}
    i = k
    j = next_vertex
    pⱼ = get_point(tri, j)
    if is_right(direction)
        return_flag, cert, i, j, k = search_right_down_adjacent_boundary_edges(tri, q, q_pos, predicates, store_history, history, ghost_vertex, i, j, pⱼ)
    else
        return_flag, cert, i, j, k = search_left_down_adjacent_boundary_edges(tri, q, q_pos, predicates, store_history, history, ghost_vertex, i, j, pⱼ)
    end
    return_flag && return cert, i, j, k
    throw(PointNotFoundError(tri, q))
end
function search_right_down_adjacent_boundary_edges(tri::Triangulation, q, q_pos, predicates::AbstractPredicateType, store_history::F, history, ghost_vertex, i, j, pⱼ) where {F}
...
function search_left_down_adjacent_boundary_edges(tri::Triangulation, q, q_pos, predicates::AbstractPredicateType, store_history::F, history, ghost_vertex, i, j, pⱼ) where {F}
    if is_true(store_history) # in case we don't enter the loop
...
"""
    check_for_intersections_with_interior_edges_adjacent_to_boundary_vertex(tri::Triangulation, k, q, right_cert, left_cert, predicates::AbstractPredicateType=def_alg222(), store_history=Val(false), history=nothing, ghost_vertex=𝒢) -> (Bool, Vertex, Vertex, Certificate, Certificate)
...
"""
function check_for_intersections_with_interior_edges_adjacent_to_boundary_vertex(tri::Triangulation, k, q, right_cert, left_cert, predicates::AbstractPredicateType=def_alg222(), store_history::F=Val(false), history=nothing, ghost_vertex=integer_type(tri)(𝒢)) where {F}
    I = integer_type(tri)
    p = get_point(tri, k)
    ...
    for _ in 1:max(num_interior_neighbours, 1)
        ...
        return_flag, _i, _j, edge_cert, triangle_cert, i, pᵢ, certᵢ = check_for_intersections_with_single_interior_edge_adjacent_to_boundary_vertex(tri, k, q, predicates, store_history, history, p, i, pᵢ, certᵢ)
...
function check_for_intersections_with_single_interior_edge_adjacent_to_boundary_vertex(tri::Triangulation, k, q, predicates::AbstractPredicateType, store_history::F, history, p, i, pᵢ, certᵢ) where {F}
    j = get_adjacent(tri, k, i)
    pⱼ = get_point(tri, j)
    certⱼ = point_position_relative_to_line(predicates, p, pⱼ, q)
    I = integer_type(tri)
    ...
    if (is_left(certᵢ) && is_right(certⱼ)) || (is_right(certᵢ) && is_left(certⱼ))
        ...
        intersection_cert = line_segment_intersection_type(predicates, p, q, pᵢ, pⱼ)
        ...
        pⱼp_edge = point_position_relative_to_line(predicates, pⱼ, p, q)
        ppᵢ_edge = point_position_relative_to_line(predicates, p, pᵢ, q)
...
function check_for_intersections_with_triangle_left_to_boundary_vertex(tri::Triangulation, k, q, left_cert, predicates::AbstractPredicateType, store_history::F, history, p, other_boundary_node, num_interior_neighbours, i, pᵢ, certᵢ) where {F}
    I = integer_type(tri)
...
"""
    exterior_find_triangle(tri::Triangulation, k, q, ghost_vertex=𝒢, predicates::AbstractPredicateType=def_alg222()) -> (Vertex, Vertex)
...
"""
function exterior_find_triangle(tri::Triangulation, k, q, ghost_vertex=integer_type(tri)(𝒢), predicates::AbstractPredicateType=def_alg222())
    ...
    if is_left(q_position) # q is left of the ghost edge through pᵢ, so rotate left 
        return exterior_find_triangle_rotate_left(tri, q, i, pₘ, ghost_vertex, predicates)
    else # rotate right 
        return exterior_find_triangle_rotate_right(tri, q, i, pₘ, ghost_vertex, predicates)
    end
end
function exterior_find_triangle_rotate_left(tri, q, i, pₘ, ghost_vertex, predicates)
    j = get_right_boundary_node(tri, i, ghost_vertex)
    pⱼ = get_point(tri, j)
    new_q_pos = point_position_relative_to_line(predicates, pₘ, pⱼ, q)
...
function exterior_find_triangle_rotate_right(tri, q, i, pₘ, ghost_vertex, predicates)
    j = get_left_boundary_node(tri, i, ghost_vertex)
    pⱼ = get_point(tri, j)
    new_q_pos = point_position_relative_to_line(predicates, pₘ, pⱼ, q)
...
For the variables defined below, you may want to refer to the extended help which also gives 
...
# Keyword Arguments 
- `predicates::AbstractPredicateType=Exact()`: Method to use for computing predicates. Can be one of [`Fast`](@ref), [`Exact`](@ref), and [`Adaptive`](@ref). See the documentation for a further discussion of these methods.
...
function find_triangle(tri::Triangulation, _q;
    ...
    _qx, _qy = getxy(_q)
    q = (G(_qx), G(_qy))
    return _find_triangle(tri, q, I(k), store_history, history, rng, maxiters, zero(maxiters), concavity_protection, zero(maxiters), use_barriers, predicates)
end

"""
    initialise_find_triangle_interior_vertex(tri::Triangulation, q, k, predicates::AbstractPredicateType, store_history::F, history, rng) -> (Bool, Point, Vertex, Vertex, Point, Point)
...
"""
function initialise_find_triangle_interior_vertex(tri::Triangulation, q, k, predicates::AbstractPredicateType, store_history::F, history, rng) where {F}
    ...
    p, i, j, pᵢ, pⱼ = select_initial_triangle_interior_vertex(tri, k, q, predicates, store_history, history, rng)
...
"""
    initialise_find_triangle_boundary_vertex(tri::Triangulation, q, k, predicates::AbstractPredicateType, store_history, history, ghost_vertex, concavity_protection) -> (Bool, Bool, Triangle, Point, Vertex, Vertex, Point, Point)
...
"""
function initialise_find_triangle_boundary_vertex(tri::Triangulation, _q, k, predicates::AbstractPredicateType, store_history::F, history, ghost_vertex, concavity_protection) where {F}
...
"""
    find_triangle_return_on_vertex(tri::Triangulation, q, k, p, pᵢ, pⱼ, i, j, predicates::AbstractPredicateType=def_alg222()) -> (Bool, Bool, Triangle)
...
"""
function find_triangle_return_on_vertex(tri::Triangulation, q, k, p, pᵢ, pⱼ, i, j, predicates)
...
"""
function find_triangle_across_triangle(tri::Triangulation, q, k, predicates::AbstractPredicateType, store_history::F, history, maxiters, cur_iter, concavity_protection, arrangement, original_k, last_changed, p, i, j, pᵢ, pⱼ) where {F}
    cur_iter += 1
    if is_true(store_history)
        if last_changed == i
    ...
    k = get_adjacent(tri, i, j)
    pₖ = get_point(tri, k)
    pₖ_pos = point_position_relative_to_line(predicates, p, q, pₖ)
    if is_right(pₖ_pos)
        if is_true(store_history)
            add_triangle!(history, i, j, k)
        end
    ...
    elseif is_left(pₖ_pos)
        ...
        in_cert = point_position_relative_to_triangle(predicates, pᵢ, pⱼ, pₖ, q) # ... Maybe there is a better way to compute this, reusing previous certificates? Not sure. ...
        if is_true(store_history)
...
"""
    find_triangle_degenerate_arrangement(tri::Triangulation, q, k, predicates::AbstractPredicateType, store_history::F, history, pᵢ, pⱼ, i, j) -> Bool
...
"""
function find_triangle_degenerate_arrangement(tri::Triangulation, q, k, predicates::AbstractPredicateType, store_history::F, history, pᵢ, pⱼ, i, j) where {F}
...
function _find_triangle(tri::Triangulation, q, k, store_history::F, history, rng::Random.AbstractRNG, maxiters, cur_iter, concavity_protection, num_restarts, use_barriers::Val{U}, predicates::AbstractPredicateType) where {F,U}
    ...
    if !(is_bnd && is_exterior_ghost_vertex(tri, ghost_vertex)) || !has_ghost_triangles(tri)
        ...
        restart_flag, return_flag, V, p, i, j, pᵢ, pⱼ = initialise_find_triangle_boundary_vertex(tri, q, k, predicates, store_history, history, ghost_vertex, concavity_protection)
    ...
    if q == p || q == pᵢ || q == pⱼ
        restart_flag, return_flag, V = find_triangle_return_on_vertex(tri, q, k, p, pᵢ, pⱼ, i, j, predicates)
        if restart_flag && !is_true(use_barriers)
            return restart_find_triangle(tri, q, store_history, history, rng, maxiters, cur_iter, concavity_protection, num_restarts + 1, use_barriers, predicates)
        elseif restart_flag && is_true(use_barriers)
            return V, true
    ...
    while is_positively_oriented(arrangement) && !reached_barrier
        restart_flag, return_flag, reinitialise_flag, V, cur_iter, arrangement, k, last_changed, original_k, pᵢ, pⱼ, i, j = find_triangle_across_triangle(tri, q, k, predicates, store_history, history, maxiters, cur_iter, concavity_protection, arrangement, original_k, last_changed, p, i, j, pᵢ, pⱼ)
        if restart_flag && !is_true(use_barriers)
            return restart_find_triangle(tri, q, store_history, history, rng, maxiters, cur_iter, concavity_protection, num_restarts + 1, use_barriers, predicates)
        elseif restart_flag && is_true(use_barriers)
            return V, true
        ...
        reinitialise_flag && return _find_triangle(tri, q, k, store_history, history, rng, maxiters, cur_iter, concavity_protection, num_restarts + 1, use_barriers, predicates)
    ...
    if is_degenerate(arrangement)
        reinitialise_flag = find_triangle_degenerate_arrangement(tri, q, k, predicates, store_history, history, pᵢ, pⱼ, i, j)
        if reinitialise_flag && !is_true(use_barriers)
            return _find_triangle(tri, q, last_changed == I(∅) ? i : last_changed, store_history, history, rng, maxiters, cur_iter, concavity_protection, num_restarts + 1, use_barriers, predicates)
...
"""
    concavity_protection_check(tri::Triangulation, concavity_protection, V, q, predicates::AbstractPredicateType=def_alg222()) -> Bool
...
"""
function concavity_protection_check(tri::Triangulation, concavity_protection, V, q, predicates::AbstractPredicateType=def_alg222())
...
"""
    restart_find_triangle(tri::Triangulation, q, store_history, history, rng, maxiters, cur_iter, concavity_protection, num_restarts, use_barriers, predicates::AbstractPredicateType) -> Triangle[, Bool]
...
"""
function restart_find_triangle(tri, q, store_history, history, rng, maxiters, cur_iter, concavity_protection, num_restarts, use_barriers, predicates::AbstractPredicateType)
