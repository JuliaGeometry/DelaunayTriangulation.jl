...
# Keyword Arguments 
- `point_indices=each_solid_vertex(tri)`: The indices of the points to be used in the [`jump_and_march`](@ref) algorithm for selecting the initial point.
- `m=default_num_samples(length(point_indices))`: The number of samples (without replacement) to be used in the [`jump_and_march`](@ref) algorithm for selecting the initial point.
- `try_points=()`: Additional points to try for selecting the initial point, in addition to the `m` sampled.
- `rng::AbstractRNG=Random.default_rng()`: The random number generator to be used in [`jump_and_march`](@ref).
- `initial_search_point=integer_type(tri)(select_initial_point(tri, new_point; point_indices, m, try_points, rng))`: The initial point to be used in [`jump_and_march`](@ref).
...
function add_point!(tri::Triangulation, new_point;
    ...
    if is_weighted(tri)
        cert = point_position_relative_to_circumcircle(tri, V, new_point) # redirects to point_position_relative_to_witness_plane
