...
"""
    is_vertex_closer_than_neighbours([predicates::AbstractPredicateType=def_alg222(),] tri::Triangulation, u, v, jᵢ, jᵢ₋₁, jᵢ₊₁) -> Bool
    is_vertex_closer_than_neighbours([predicates::AbstractPredicateType=def_alg222(),] tri::Triangulation, list::ShuffledPolygonLinkedList, u, v, j) -> Bool
...
"""
function is_vertex_closer_than_neighbours(predicates::AbstractPredicateType, tri::Triangulation, u, v, jᵢ, jᵢ₋₁, jᵢ₊₁)
...
function is_vertex_closer_than_neighbours(predicates::AbstractPredicateType, tri::Triangulation, list::ShuffledPolygonLinkedList, u, v, j)
    jᵢ, jᵢ₋₁, jᵢ₊₁ = get_triplet(list, j)
    return is_vertex_closer_than_neighbours(predicates, tri, u, v, jᵢ, jᵢ₋₁, jᵢ₊₁)
end
is_vertex_closer_than_neighbours(tri::Triangulation, u, v, jᵢ, jᵢ₋₁, jᵢ₊₁) = is_vertex_closer_than_neighbours(def_alg222(), tri::Triangulation, u, v, jᵢ, jᵢ₋₁, jᵢ₊₁)
is_vertex_closer_than_neighbours(tri::Triangulation, list::ShuffledPolygonLinkedList, u, v, j) = is_vertex_closer_than_neighbours(def_alg222(), tri::Triangulation, list::ShuffledPolygonLinkedList, u, v, j)
...
"""
function select_random_vertex(tri::Triangulation, list::ShuffledPolygonLinkedList, u, v, range, rng::Random.AbstractRNG, predicates::AbstractPredicateType)
    j = rand(rng, range)
...
"""
    delete_polygon_vertices_in_random_order!(list::ShuffledPolygonLinkedList, tri::Triangulation, u, v, rng::Random.AbstractRNG, predicates::AbstractPredicateType)
...
"""
function delete_polygon_vertices_in_random_order!(list::ShuffledPolygonLinkedList, tri::Triangulation, u, v, rng::Random.AbstractRNG, predicates::AbstractPredicateType)
    m = list.k
    for i in (m-1):-1:3
...
"""
    setup_cavity_cdt(tri::Triangulation, V; rng::Random.AbstractRNG=Random.default_rng(), predicates::AbstractPredicateType=def_alg222()) -> ShuffledPolygonLinkedList
...
"""
function setup_cavity_cdt(tri::Triangulation, V; rng::Random.AbstractRNG=Random.default_rng(), predicates::AbstractPredicateType=def_alg222())
    v = V[begin]
    u = V[end]
...
"""
    triangulate_cavity_cdt!(tri::Triangulation, V, marked_vertices; rng::Random.AbstractRNG=Random.default_rng(), predicates::AbstractPredicateType=def_alg222())
...
"""
function triangulate_cavity_cdt!(tri::Triangulation, V, tri_fan::Triangulation, marked_vertices, fan_triangles; rng::Random.AbstractRNG=Random.default_rng(), predicates::AbstractPredicateType=def_alg222())
...
"""
    retriangulate_fan!(tri::Triangulation, tri_fan::Triangulation, fan, fan_triangles; predicates::AbstractPredicateType=def_alg222(), rng::Random.AbstractRNG=Random.default_rng())
...
"""
function retriangulate_fan!(tri::Triangulation, tri_fan::Triangulation, fan, fan_triangles; predicates::AbstractPredicateType=def_alg222(), rng::Random.AbstractRNG=Random.default_rng())
...
"""
function add_point_cavity_cdt!(tri::Triangulation, u, v, w, marked_vertices, predicates::AbstractPredicateType=def_alg222())
...
"""
    locate_intersecting_triangles(tri::Triangulation, e, rotate=Val(true), rng::Random.AbstractRNG=Random.default_rng(), predicates::AbstractPredicateType=def_alg222()) -> (Vector, Vector, Vector, Vector)
...
"""
function locate_intersecting_triangles(tri::Triangulation, e, rotate=Val(true), rng::Random.AbstractRNG=Random.default_rng(), predicates::AbstractPredicateType=def_alg222())
    V = triangle_type(tri)
    E = edge_type(tri)
    I = integer_type(tri)
...
"""
    process_intersecting_triangles!(tri::Triangulation, e, collinear_segments; predicates::AbstractPredicateType=def_alg222(), rng::Random.AbstractRNG=Random.default_rng()) -> Bool
...
The `predicates::AbstractPredicateType` argument defines the method for computing predicates. Can be one of [`Fast`](@ref), [`Exact`](@ref), and [`Adaptive`](@ref). See the documentation for a further discussion of these methods.

See also [`connect_segments!`](@ref), [`extend_segments!`](@ref), [`split_segment!`](@ref) and [`split_boundary_edge_at_collinear_segments!`](@ref).
"""
function process_collinear_segments!(tri::Triangulation, e, collinear_segments; predicates::AbstractPredicateType=def_alg222(), rng::Random.AbstractRNG=Random.default_rng())
...
"""
    constrained_triangulation!(tri::Triangulation, segments, boundary_nodes, predicates::AbstractPredicateType, full_polygon_hierarchy; rng=Random.default_rng(), delete_holes=true) -> Triangulation
...
"""
function constrained_triangulation!(tri::Triangulation, segments, boundary_nodes, predicates::AbstractPredicateType, full_polygon_hierarchy; rng=Random.default_rng(), delete_holes=true)
