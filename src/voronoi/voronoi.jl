...
function polygon_features(vor::VoronoiTessellation, i)
    ...
    return polygon_features(polygon, get_polygon_points(vor))
end

get_area(vor::VoronoiTessellation, i) = polygon_features(vor, i)[1]
get_centroid(vor::VoronoiTessellation, i) = get_polygon_point(vor, polygon_features(vor, i))[2]
...
## Clipping  
struct _ClippingQueue{E,I,F}
    edges_to_process::Set{E}
    polygon_edge_queue::Queue{Tuple{E,I}}
    boundary_sites::Dict{I,Set{I}} # maps polygon to intersection points in segment_intersections
    segment_intersections::Vector{NTuple{2,F}}
    processed_pairs::Set{Tuple{E,I}}
end
function _ClippingQueue(vorn::VoronoiTessellation)
    ...
    processed_pairs = Set{Tuple{E,I}}()
    return _ClippingQueue{E,I,F}(edges_to_process, polygon_edge_queue, boundary_sites, segment_intersections, processed_pairs)
end

function mark_as_processed!(cq::_ClippingQueue, e, i)
    if contains_edge(e, cq.edges_to_process)
        delete!(cq.edges_to_process, e)
        #push!(cq.processed_pairs, (e, i))
    elseif contains_edge(reverse_edge(e), cq.edges_to_process)
        delete!(cq.edges_to_process, reverse_edge(e))
        #push!(cq.processed_pairs, (reverse_edge(e), i))
    end
    return nothing
end
pair_is_processed(cq::_ClippingQueue, e, i) = (e, i) ∈ cq.processed_pairs || (reverse_edge(e), i) ∈ cq.processed_pairs
edge_is_processed(cq::_ClippingQueue, e) = !contains_edge(e, cq.edges_to_process) && !contains_edge(reverse_edge(e), cq.edges_to_process)
enqueue_polygon_and_edge!(cq::_ClippingQueue, e, i) = enqueue!(cq.polygon_edge_queue, (e, i))
dequeue_polygon_and_edge!(cq::_ClippingQueue) = dequeue!(cq.polygon_edge_queue)
function get_incident_polygon(vor::VoronoiTessellation, e)
    i, j = edge_indices(e)
    p, q = get_generator(vor, i, j)
    px, py = getxy(p)
    qx, qy = getxy(q)
    m = (px + qx) / 2, (py + qy) / 2
    closest_polygon = jump_and_march(vor, m; k=i)
    return closest_polygon
end
all_edges_processed(cq::_ClippingQueue) = isempty(cq.edges_to_process)
queue_is_empty(cq::_ClippingQueue) = isempty(cq.polygon_edge_queue)
function add_segment_intersection!(cq::_ClippingQueue{E,I,F}, p, i) where {E,I,F}
    segment_intersections = cq.segment_intersections
    boundary_sites = cq.boundary_sites
...
get_edge_to_process(cq::_ClippingQueue) = first(cq.edges_to_process)
...
function initialise_clipping_queue(vorn::VoronoiTessellation)
    clipping_queue = _ClippingQueue(vorn)
    tri = get_triangulation(vorn)
    E = edge_type(tri)
    intersected_edge_cache = Vector{E}(undef, 2)
    return clipping_queue, intersected_edge_cache
end

function queue_initial_edge!(vorn::VoronoiTessellation, clipping_queue)
    e = get_edge_to_process(clipping_queue)
    closest_polygon = get_incident_polygon(vorn, e)
    enqueue_polygon_and_edge!(clipping_queue, e, closest_polygon)
    return nothing
end
...
function process_ray_intersection!(
    ...
    qx, qy = getxy(q)
    m = (px + qx) / 2, (py + qy) / 2
    add_segment_intersection!(clipping_queue, m, incident_polygon)
    num_intersections += 1
    tri = get_triangulation(vorn)
    E = edge_type(tri)
...
function process_queue_with_intersection!(clipping_queue, e, incident_polygon, intersected_edge_cache, num_intersections, vorn::VoronoiTessellation, left_bnd, right_bnd)
    if iszero(num_intersections)
        ...
        if !interior_intersection
            ...
            enqueue_polygon_and_edge!(clipping_queue, other_e, adjacent_incident_polygon)
            # Need to also add the corner point between the two edges.
            add_segment_intersection!(clipping_queue, getxy(get_generator(vorn, shared_vertex)), incident_polygon)
        else
            enqueue_polygon_and_edge!(clipping_queue, e, left_bnd)
            enqueue_polygon_and_edge!(clipping_queue, e, right_bnd)
        end
    end
    return nothing
end
...
function process_next_edge!(vorn::VoronoiTessellation, clipping_queue, intersected_edge_cache)
    e, incident_polygon = dequeue_polygon_and_edge!(clipping_queue)
    @show e, incident_polygon
    ## TODO: How can I amend this code so that I only have to process the edge e once? I really don't believe I need to mark (e, incident_polygon) instead of just e.
    ## If only the paper had any code with it! Why do so many computational geometry papers have no code?!
    edge_is_processed(clipping_queue, e) && return false
    mark_as_processed!(clipping_queue, e, incident_polygon)
    ...
    for ℓ in 1:nedges
        ...
        if is_boundary_index(u) && is_boundary_index(v)
            continue
        elseif is_boundary_index(u) && !is_boundary_index(v) # ray coming back 
            num_intersections = process_ray_intersection!(vorn, u, v, clipping_queue, incident_polygon, num_intersections, intersected_edge_cache)
        elseif !is_boundary_index(u) && is_boundary_index(v) # ray leaving
            num_intersections = process_ray_intersection!(vorn, v, u, clipping_queue, incident_polygon, num_intersections, intersected_edge_cache)
        else # finite segment
            for e in (e, left_edge, right_edge)
                _num_intersections = process_segment_intersection!(vorn, u, v, e, clipping_queue, incident_polygon, num_intersections, intersected_edge_cache)
    ...
    if num_intersections ≠ 0
        process_queue_with_intersection!(clipping_queue, e, incident_polygon, intersected_edge_cache, num_intersections, vorn, left_bnd, right_bnd)
    end
    return true
end

function find_boundary_sites(vorn::VoronoiTessellation)
    clipping_queue, intersected_edge_cache = initialise_clipping_queue(vorn)
    queue_initial_edge!(vorn, clipping_queue)
    while !all_edges_processed(clipping_queue)
        if queue_is_empty(clipping_queue)
            queue_initial_edge!(vorn, clipping_queue)
        end
        process_next_edge!(vorn, clipping_queue, intersected_edge_cache)
    end
    return clipping_queue
end

function update_polygon_vertices!(vorn::VoronoiTessellation, clipping_queue)
    n = num_polygon_vertices(vorn)
    boundary_sites = clipping_queue.boundary_sites
    for (polygon, new_verts) in boundary_sites
        polygon_vertices = get_polygon(vorn, polygon)
        unique!(polygon_vertices)
        ## TODO: Check how we can amend the clipping code so that δ doesn't have to be computed at all.
        @show polygon_vertices
        filter!(v -> begin
                tri = get_triangulation(vorn)
                return !(
                    is_boundary_index(v) ||
                    distance_to_polygon(get_polygon_point(vorn, v), get_points(tri), get_boundary_nodes(tri)) < zero(number_type(vorn))
                )
            end, polygon_vertices)
        @show polygon_vertices
        for i in new_verts
            push!(polygon_vertices, n + i)
        end
    end
end

function add_intersection_points!(vorn::VoronoiTessellation, clipping_queue)
    segment_intersections = clipping_queue.segment_intersections
...
function arithmetic_average_unsorted(vorn::VoronoiTessellation, polygon_vertices) # polygons aren't sorted here, and they are not is_circular
    ...
    for i in eachindex(polygon_vertices)
...
function sort_convex_polygon!(vorn::VoronoiTessellation, polygon) # polygons aren't sorted here, and they are not is_circular
    polygon_vertices = get_polygon(vorn, polygon)
    n = length(polygon_vertices)
    F = number_type(vorn)
    ...
    for i in eachindex(polygon_vertices)
        p = get_polygon_point(vorn, i)
        px, py = getxy(p)
        θ[i] = atan(py - cy, px - cx)
    end
    idx = sortperm(θ)
    permute!(polygon_vertices, idx)
    push!(polygon_vertices, polygon_vertices[begin]) # fix circularity
    return nothing
end

function clip_voronoi_tessellation!(vorn::VoronoiTessellation)
    clipping_queue = find_boundary_sites(vorn)
    update_polygon_vertices!(vorn, clipping_queue)
    add_intersection_points!(vorn, clipping_queue)
    for polygon in each_polygon_index(vorn)
        sort_convex_polygon!(vorn, polygon)
    end
