function clip_polygon(vertices, points, clip_vertices, clip_points; predicates::AbstractPredicateKernel = DEFAULT_KERNEL)
    P = Polygon(vertices, points)
    Q = Polygon(clip_vertices, clip_points)
    return clip_polygon(P, Q; predicates)
end
...
function clip_polygon(poly::Polygon{T}, clip_poly::Polygon; predicates::AbstractPredicateKernel = DEFAULT_KERNEL) where {T}
    output_list = poly
    q = clip_poly[end]
    for p in clip_poly
