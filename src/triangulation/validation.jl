...
function Base.summary(state::TriangleOrientationState)
    if test_state(state)
        ...
        return lazy"The triangle $(state.bad_triangle) is $(state.triangle_orientation) oriented."
    end
end
...
function Base.summary(state::DelaunayCriterionState)
    if test_state(state)
        ...
        if state.bad_vertex !== ∅
            return lazy"The Delaunay criterion does not hold for the triangle-vertex pair ($(state.bad_triangle), $(state.bad_vertex))."
        else
...
function Base.summary(state::EdgesHaveTwoIncidentTrianglesState)
    if test_state(state)
        ...
        return lazy"The edge $(state.bad_edge) does not have two incident triangles."
    end
end
...
function Base.summary(state::AdjacentMapState)
    if test_state(state)
        ...
        return lazy"The triangle $(state.bad_triangle) is incorrectly stored in the adjacent map, with the edge $(state.bad_edge) instead mapping to $(state.bad_vertex)."
    end
end
...
function Base.summary(state::Adjacent2VertexMapState)
    if test_state(state)
        ...
        return lazy"The adjacent2vertex map is incorrect for the triangle $(state.bad_triangle), as the edge $(state.bad_edge) is not inside the vertex $(state.bad_vertex)'s associated edge set in the map, $(state.bad_edge_set)."
    end
end
...
function Base.summary(state::AdjacentMapAdjacent2VertexMapState)
    if test_state(state)
        ...
        return lazy"The adjacent2vertex map is inconsistent with the adjacent map. The vertex $(state.bad_vertex) and edge $(state.bad_edge) are inconsistent with the associated edge set $(state.bad_edge_set) from the adjacent2vertex map, as $(state.bad_edge) maps to $(state.bad_vertex2) instead of $(state.bad_vertex)."
    end
end
...
function Base.summary(state::Adjacent2VertexMapAdjacentMapState)
    if test_state(state)
        ...
        if !state.in_adjacent2vertex
            return lazy"The adjacent2vertex map is inconsistent with the adjacent map. The edge $(state.bad_edge) is mapped to $(state.bad_vertex) but $(state.bad_vertex) is not a key in the adjacent2vertex map."
        else
            return lazy"The adjacent2vertex map is inconsistent with the adjacent map. The edge $(state.bad_edge) is mapped to $(state.bad_vertex) but $(state.bad_edge) is not in the edge set that $(state.bad_vertex) maps to in the adjacent2vertex map."
        end
    end
end
...
function Base.summary(state::GraphState)
    if test_state(state)
        ...
        return lazy"The graph does not include the vertex $(state.bad_vertex) despite it being in the triangulation."
    end
end
...
function Base.summary(state::GraphAdjacentMapState)
    if test_state(state)
        ...
        return lazy"The edge $(state.bad_edge) appears as an edge in the graph but it and its reverse are not both a key of the adjacent map."
    end
end
...
function Base.summary(state::AdjacentMapGraphState)
    if test_state(state)
        ...
        if state.is_k
            return lazy"The vertex $(state.bad_vertex) is adjacent to the edge $(state.bad_edge) but either $(state.bad_edge_2[1]) or $(state.bad_edge_2[2]) fail to be in $(state.bad_vertex)'s neighbourhood."
        else
            return lazy"The vertex $(state.bad_vertex), which defines the edge $(state.bad_edge) with adjacent vertex $(state.bad_vertex), is inconsistent with the graph which fails to include at least one of $(state.bad_edge_2[1]) and $(state.bad_edge_2[2]) in $(state.bad_vertex)'s neighbourhood."
        end
    end
end
...
function Base.summary(state::GraphTrianglesState)
    if test_state(state)
        ...
        if state.bad_vertex == ∅
            return lazy"The graph is inconsistent with the triangle set, as one of the vertices of $(state.bad_triangle) is not a vertex in the graph."
        else
            return lazy"The graph is inconsistent with the triangle set. The triangle $(state.bad_triangle) is in the triangle set but either $(state.bad_edge[1]) or $(state.bad_edge[2]) are not in $(state.bad_vertex)'s neighbourhood."
        end
    end
end
...
function Base.summary(state::SegmentState)
    if test_state(state)
        ...
        if state.type == 0
            return lazy"The segment $(state.segment) is not in the triangulation."
        elseif state.type == 1
            return lazy"The segment $(state.segment) is in the triangulation, but it was not in the interior_segment field and not in the boundary edges."
        elseif state.type == 2
            return lazy"The interior segments are not a subset of all segments."
        end
    end
end
...
function Base.summary(state::BoundaryEdgeMapBoundaryNodesState)
    if test_state(state)
        return lazy"The boundary edge map is consistent with the boundary nodes."
    else
        return lazy"The boundary edge map is inconsistent with the boundary nodes. The edge $(state.bad_edge) maps to $(state.bad_pos) in the boundary edge map but $(state.bad_pos) corresponds to the edge $(state.bad_edge_2)."
    end
end
...
function Base.summary(state::BoundaryNodesBoundaryEdgeMapState)
    if test_state(state)
        ...
        return lazy"The boundary nodes are inconsistent with the boundary edge map. The edge $(state.bad_edge) maps to $(state.bad_pos_2) but is at $(state.bad_pos)."
    end
end
...
function Base.summary(state::UniqueIteratorOutputState)
    if test_state(state)
        return lazy"The $(state.iterator_name) iterator has no duplicate $(state.primitive_name)s."
    else
        return lazy"The $(state.iterator_name) iterator has duplicate $(state.primitive_name)s."
    end
end
...
function Base.summary(state::IteratorLengthState)
    if test_state(state)
        return lazy"The $(state.iterator_name) iterator correctly has the same length as the number of $(state.primitive_name)s in the triangulation."
    else
        return lazy"The $(state.iterator_name) iterator has a length $(state.iterator_length), different to the number of $(state.primitive_name)s ($(state.correct_length)) returned from $(state.iterator_function_name)."
    end
end
...
function Base.summary(state::IteratorOutputState)
    if test_state(state)
        return lazy"The $(state.iterator_name) iterator returns the same set of $(state.primitive_name)s as returned from $(state.iterator_function_name)."
    else
        return lazy"The $(state.iterator_name) iterator returns a different set of $(state.primitive_name)s as returned from $(state.iterator_function_name)."
    end
end
...
function Base.summary(state::IteratorState)
    test_state(state) && return lazy"The $(state.iterator_name) iterator is correctly constructed."
...
function test_iterators(tri::Triangulation)
    ...
    sort!(solid_vertices)
    sort!(ghost_vertices)
    triangle_output_flag = sort(sort_triangle.(all_triangles)) == sort(sort_triangle.(collect(each_triangle(tri))))
    solid_triangle_output_flag = sort(sort_triangle.(solid_triangles)) == sort(sort_triangle.(collect(each_solid_triangle(tri))))
    ghost_triangle_output_flag = sort(sort_triangle.(ghost_triangles)) == sort(sort_triangle.(collect(each_ghost_triangle(tri))))
...
function Base.summary(state::DuplicateSegmentsState)
    if test_state(state)
        ...
        if state.interior
            return lazy"The edge $(state.bad_edge) is duplicated in the interior segments field."
        else
            return lazy"The edge $(state.bad_edge) is duplicated in the all segments field."
        end
    end
end
