...
function triangulate_bowyer(pts;
    ...
    DG = DelaunayGraph{I}()
    initial_triangle = construct_positively_oriented_triangle(TriangleType,
    ...
    for (num_points, new_point) in enumerate(@view pt_order[(begin+3):end])
        ...
        initial_search_point = select_initial_point(pts, new_point; pt_idx, m, try_points=try_last_inserted_point ? last_inserted_point : ())
