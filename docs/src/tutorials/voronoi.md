```@meta
EditURL = "https://github.com/DanielVandH/DelaunayTriangulation.jl/tree/main/docs/src/literate_tutorials/voronoi.jl"
```
...
a dual Delaunay triangulation using `voronoi`. To start, let us
...
may not contain all the points, noting that `get_generators(vorn)` is just a repackaged
...
It is preferred that you use `each_generator(vorn)`, though, which is an iterator
over the generators:
...
Note that this is just the keys of the above `Dict`. To access specific generators, you use `get_generator`. For example,
...
look at a specific polygon, you should use `get_polygon`. For example,
...
the `polygon_points` field, which you could then obtain using `get_polygon_point`. For example,
...
polygon, then you can use `each_polygon`:
...
on these two iterators. This would be the same as iterating over the interal `polygons`
field, but this could be subject to change in the future. To get an iterator over the polygon
vertices rather than caring about a specific polygon, you use `each_polygon_vertex`:
...
Given an edge in the tessellation, you can use `get_adjacent` to get the polygon
...
You can view the source code for this file [here](https://github.com/DanielVandH/DelaunayTriangulation.jl/tree/new-docs/docs/src/literate_tutorials/voronoi.jl).
