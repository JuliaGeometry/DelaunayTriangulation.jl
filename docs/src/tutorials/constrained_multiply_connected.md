```@meta
EditURL = "https://github.com/DanielVandH/DelaunayTriangulation.jl/tree/main/docs/src/literate_tutorials/constrained_multiply_connected.jl"
```
...
Like before, we examine individual segments by referring to them by their boundary indices,
...
tutorials since there are many more boundary indices. This is where the boundary fields
...
edge within the segment. To look at the boundary indices directly, another useful field is
...
This field maps a boundary index to the complete boundary indices that might be found on the
curve corresponding to that boundary index. For example, `-3 => -4:-3` means that the boundary
index `-3` is part of a curve that, in addition to itself, contains the boundary index `-4`.
If you want all the boundary indices, you can use

````@example constrained_multiply_connected
DelaunayTriangulation.all_boundary_indices(tri)
````

which is just `keys(get_boundary_index_ranges(tri))`. If you just want to find
what curve and what segment a boundary index belongs to, you can look at the `boundary_map`:

````@example constrained_multiply_connected
get_boundary_map(tri)
````

So that, for example, `-3 => (2, 1)` means that the boundary index `-3` corresponds
to the first segment (from the second `Tuple` element) of the second curve (from the
first `Tuple` element).
...
function get_triangulation_area(tri)
    ...
    for curve_index in 1:nc
        ...
        ns = DelaunayTriangulation.num_segments(bn)
...
function get_perimeters(tri)
    ...
    for (e, ((curve_index, segment_index), node_index)) in get_boundary_edge_map(tri)
        u, v = edge_indices(e)
        p, q = get_point(tri, u, v)
        ℓ = sqrt((getx(p) - getx(q))^2 + (gety(p) - gety(q))^2)
        total_perimeter += ℓ
        curve_perimeters[curve_index] += ℓ
        if haskey(segment_perimeters, (curve_index, segment_index))
            segment_perimeters[(curve_index, segment_index)] += ℓ
        else
            segment_perimeters[(curve_index, segment_index)] = ℓ
        end
    end
...
You can view the source code for this file [here](https://github.com/DanielVandH/DelaunayTriangulation.jl/tree/new-docs/docs/src/literate_tutorials/constrained_multiply_connected.jl).
