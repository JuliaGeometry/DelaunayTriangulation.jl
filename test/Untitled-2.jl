...
function test_adjacent_map_matches_triangles(tri)
    ...
    DT.clear_empty_features!(tri)
    return AdjacentMapState(true, (DT.∅, DT.∅, DT.∅), DT.∅)
end
...
function test_adjacent2vertex_map_matches_triangles(tri)
    E = DT.edge_type(tri)
    for T in each_triangle(tri)
        for (u, v, w) in DT.triangle_edges(T)
...
function test_adjacent_map_matches_adjacent2vertex_map(tri)
    for (k, S) in get_adjacent2vertex(get_adjacent2vertex(tri))
        ...
        for e in each_edge(S)
            ...
            !flag && return AdjacentMapAdjacent2VertexMapState(flag, false, Int(k), get_adjacent(tri, e), _S, Int.(edge_vertices(e)))
        end
    end
    DT.clear_empty_features!(tri)
    return AdjacentMapAdjacent2VertexMapState(true, false, DT.∅, DT.∅, Set{NTuple{2,Int}}(), (DT.∅, DT.∅))
end
...
function Base.summary(state::IteratorState)
    test_state(state) && return "The $(state.iterator_name) is correctly constructed."
...
function test_state(triangulation_state::TriangulationState)
    for f in fieldnames(typeof(foo))
...
function Base.show(io::IO, triangulation_state::TriangulationState)
    ...
    if is_valid
        ...
        for f in fieldnames(typeof(foo))
            state = getfield(triangulation_state, f)
            !test_state(state) && show(io, state)
        end
    end
    return
end

function validate_triangulation(tri; print_result=false)
    state = TriangulationState(tri)
    print_result && println(state)
