...
@testset "Finding points outside" begin
    q = [-3.94232, 15.6067]
    for k in k_list
        i, j = DT.exterior_jump_and_march(pts, adj, boundary_index_ranges, rep, boundary_map, k, q)
...
@testset "Testing points that are already in the triangulation" begin
    for k in each_point_index(pts)
        if DT.is_outer_boundary_node(tri, k)
            i, j = DT.exterior_jump_and_march(pts, adj, boundary_index_ranges, rep, boundary_map, k,
                get_point(pts, rep, boundary_map, k))
            @test k ∈ (i, j) &&
