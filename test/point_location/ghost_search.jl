...
q = [-3.94232, 15.6067]
for k in k_list
    i, j = DT.exterior_jump_and_march(pts, adj, boundary_index_ranges, boundary_map, k, q)
    @test DT.is_boundary_index(get_adjacent(adj, i, j))
...
for k in each_point_index(pts)
    if DT.is_outer_boundary_node(tri, k)
        i, j = DT.exterior_jump_and_march(pts, adj, boundary_index_ranges, boundary_map, k,
                                          get_point(pts, boundary_map, k))
        @test k ∈ (i, j) &&
              DT.is_outer_ghost_triangle(i, j, get_adjacent(adj, i, j), boundary_map)
