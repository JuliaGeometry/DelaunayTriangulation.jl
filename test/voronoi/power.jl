...
fig, ax, sc = voronoiplot(vorn, clip = (-10, 10, -10, 10))
# random points in [-10, 10] x [-10, 10]
randx = rand(1000) * 20 .- 10
randy = rand(1000) * 20 .- 10
...
scatter!(ax, randpts, color = colors[neighs])
fig




points = [(-1.0, 0.0), (0.0, 0.0), (0.2, 0.25), (-1.0, 1.0)]
weights = [0.0, -1.0, 2.3, 0.0]
tri = triangulate(points; weights, randomise=false)

fig, ax, sc = triplot(tri)
c = DT.triangle_orthocenter(tri, (1, 2, 3))

# Clipped coordinates
PT = AdaptiveKernel
tri = example_triangulation()
weights = [-2.3639, -5.7554, -6.1096, 8.9622, 2.0206, -5.7043, 8.7135, -0.535]
tri = triangulate(get_points(tri); weights, predicates=PT())
vorn = voronoi(tri)
...
for T in each_solid_triangle(tri)
    c = DT.triangle_orthocenter(tri, T)
    scatter!(ax, [c], color=:red)
end
fig
=#